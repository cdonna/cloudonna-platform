...
import { ArrowRight, Compass, HandshakeIcon, Wrench } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const title = `${dict.forPartners.metaTitle}: ClouDonna`;
  const description = dict.forPartners.metaDescription;
  return {
    title,
    description,
    alternates: localizedAlternates(locale, "/for-partners"),
    openGraph: localizedOpenGraph(locale, title, description),
  };
}

const ICONS = [Compass, Wrench, HandshakeIcon];
  ...
  return (
    <div className="min-h-dvh bg-void">
      <div className="mx-auto max-w-3xl px-6 pb-24 pt-14">
        ...
        <p className="mt-5 text-lg leading-8 text-nova-ink-muted">{dict.forPartners.sub}</p>

        <div className="mt-14 space-y-6">
          {dict.forPartners.points.map((point, index) => {
        ...
        <div className="mt-14 rounded-3xl border border-titanium bg-carbon-2 p-8">
          <h2 className="text-xl font-semibold text-nova-ink">
            {dict.forPartners.statusHeading}
          </h2>
          <p className="mt-3 text-sm leading-6 text-nova-ink-muted">{dict.forPartners.statusBody}</p>
        </div>

        <div className="mt-10 text-center">
          <Link
            href={`/${locale}/contact?type=partner`}
