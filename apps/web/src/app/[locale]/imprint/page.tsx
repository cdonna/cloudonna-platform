...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const title = `${dict.legal.imprint.metaTitle} — ClouDonna`;
  const description = dict.legal.imprint.metaDescription;
  return {
    title,
    description,
    alternates: localizedAlternates(locale, "/imprint"),
    ...
    <div className="min-h-dvh bg-obsidian">
      <div className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="mt-12">
          ...
          <h1 className="mt-5 text-4xl font-semibold tracking-[-0.03em] text-nova-ink">{i.h1}</h1>

          <p className="mt-3 text-sm text-nova-ink-faint">{i.notComplete}</p>

          <div className="mt-10 space-y-8 text-sm leading-7 text-nova-ink-muted">
            <p>{i.intro}</p>

            <section>
              <h2 className="text-lg font-semibold text-nova-ink">{i.missingHeading}</h2>
              <ul className="mt-3 list-disc space-y-1.5 pl-5">
                {i.missingFields.map((field) => (
                  <li key={field}>{field}</li>
                ))}
              </ul>
              <p className="mt-3">{i.missingFooter}</p>
            </section>

            <section>
              <h2 className="text-lg font-semibold text-nova-ink">{i.contactHeading}</h2>
              <p className="mt-2">
                {i.contactBodyPrefix}{" "}
